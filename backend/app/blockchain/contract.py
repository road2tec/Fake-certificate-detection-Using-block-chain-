"""
Smart contract interaction module for Certificate Verification.
"""
from web3 import Web3
from .web3_client import get_web3, get_default_account
from ..config.settings import get_settings
import json
from pathlib import Path

settings = get_settings()

# Contract ABI (Updated for CertificateVerification)
CONTRACT_ABI = [
    {
      "anonymous": False,
      "inputs": [
        {"indexed": True, "internalType": "bytes32", "name": "certificateHash", "type": "bytes32"},
        {"indexed": True, "internalType": "address", "name": "issuer", "type": "address"}
      ],
      "name": "CertificateIssued",
      "type": "event"
    },
    {
      "anonymous": False,
      "inputs": [
        {"indexed": True, "internalType": "bytes32", "name": "certificateHash", "type": "bytes32"},
        {"indexed": False, "internalType": "bool", "name": "exists", "type": "bool"}
      ],
      "name": "CertificateVerified",
      "type": "event"
    },
    {
      "inputs": [{"internalType": "bytes32", "name": "certificateHash", "type": "bytes32"}],
      "name": "getCertificateIssuer",
      "outputs": [{"internalType": "address", "name": "", "type": "address"}],
      "stateMutability": "view",
      "type": "function"
    },
    {
      "inputs": [{"internalType": "bytes32", "name": "certificateHash", "type": "bytes32"}],
      "name": "issueCertificate",
      "outputs": [{"internalType": "bool", "name": "", "type": "bool"}],
      "stateMutability": "nonpayable",
      "type": "function"
    },
    {
      "inputs": [{"internalType": "bytes32", "name": "certificateHash", "type": "bytes32"}],
      "name": "verifyCertificate",
      "outputs": [{"internalType": "bool", "name": "", "type": "bool"}],
      "stateMutability": "view",
      "type": "function"
    }
]

# NOTE: Bytecode below is a placeholder. 
# In a real scenario, this would be re-compiled from CertificateVerification.sol
CONTRACT_BYTECODE = ""

contract_instance = None
contract_address = None


async def deploy_contract():
    """Deploy the smart contract to Ganache."""
    global contract_instance, contract_address
    
    w3 = get_web3()
    if not w3 or not w3.is_connected():
        print("Cannot deploy contract: Not connected to blockchain")
        return None
    
    if settings.private_key:
        account = w3.eth.account.from_key(settings.private_key)
        address = account.address
    else:
         address = get_default_account()

    if not address:
        print("Cannot deploy contract: No accounts available")
        return None
    
    try:
        # Create contract
        Contract = w3.eth.contract(abi=CONTRACT_ABI, bytecode=CONTRACT_BYTECODE)
        
        # Build transaction
        nonce = w3.eth.get_transaction_count(address)
        transaction = Contract.constructor().build_transaction({
            'from': address,
            'gas': 3000000,
            'gasPrice': w3.eth.gas_price,
            'nonce': nonce,
        })

        if settings.private_key:
            signed_tx = w3.eth.account.sign_transaction(transaction, private_key=settings.private_key)
            tx_hash = w3.eth.send_raw_transaction(signed_tx.rawTransaction)
        else:
            tx_hash = w3.eth.send_transaction(transaction)

        tx_receipt = w3.eth.wait_for_transaction_receipt(tx_hash)
        
        contract_address = tx_receipt.contractAddress
        contract_instance = w3.eth.contract(address=contract_address, abi=CONTRACT_ABI)
        
        print(f"Certificate Contract deployed at: {contract_address}")
        return contract_address
        
    except Exception as e:
        print(f"Contract deployment error: {str(e)}")
        return None


def get_contract():
    """Get the deployed contract instance."""
    global contract_instance
    return contract_instance


def set_contract_address(address: str):
    """Set contract address from environment."""
    global contract_instance, contract_address
    
    w3 = get_web3()
    if w3 and w3.is_connected() and address:
        contract_address = address
        contract_instance = w3.eth.contract(address=address, abi=CONTRACT_ABI)
        return True
    return False


async def register_certificate_on_blockchain(certificate_hash: str) -> dict:
    """
    Register a certificate hash on the blockchain.
    """
    global contract_instance, contract_address
    
    w3 = get_web3()
    if not w3 or not w3.is_connected():
        return {"success": False, "error": "Not connected to blockchain"}
    
    if not contract_instance:
        if settings.contract_address:
            set_contract_address(settings.contract_address)
        else:
            await deploy_contract()
        
        if not contract_instance:
             return {"success": False, "error": "Contract not deployed"}

    try:
        # Convert hex string to bytes32
        hash_bytes = bytes.fromhex(certificate_hash)
        
        if settings.private_key:
            account = w3.eth.account.from_key(settings.private_key)
            from_address = account.address
        else:
            from_address = get_default_account()
        
        # Build transaction
        nonce = w3.eth.get_transaction_count(from_address)
        transaction = contract_instance.functions.issueCertificate(hash_bytes).build_transaction({
            'from': from_address,
            'gas': 1000000,
            'gasPrice': w3.eth.gas_price,
            'nonce': nonce,
        })
        
        if settings.private_key:
            signed_tx = w3.eth.account.sign_transaction(transaction, private_key=settings.private_key)
            tx_hash = w3.eth.send_raw_transaction(signed_tx.rawTransaction)
        else:
            tx_hash = w3.eth.send_transaction(transaction)

        # Wait for transaction receipt
        tx_receipt = w3.eth.wait_for_transaction_receipt(tx_hash)
        
        return {
            "success": True,
            "tx_hash": tx_hash.hex(),
            "block_number": tx_receipt.blockNumber
        }
        
    except Exception as e:
        print(f"Blockchain error: {str(e)}")
        return {"success": False, "error": str(e)}


async def verify_certificate_on_blockchain(certificate_hash: str) -> dict:
    """
    Verify if a certificate hash exists on the blockchain.
    """
    global contract_instance
    
    w3 = get_web3()
    if not w3 or not w3.is_connected():
        return {"success": False, "exists": False, "error": "Not connected to blockchain"}
    
    if not contract_instance:
        if settings.contract_address:
            set_contract_address(settings.contract_address)
        else:
            return {"success": False, "exists": False, "error": "Contract not deployed"}
    
    try:
        # Convert hex string to bytes32
        hash_bytes = bytes.fromhex(certificate_hash)
        
        # Call verifyCertificate function
        exists = contract_instance.functions.verifyCertificate(hash_bytes).call()
        
        result = {
            "success": True,
            "exists": exists
        }
        
        if exists:
            # Get issuer address
            owner = contract_instance.functions.getCertificateIssuer(hash_bytes).call()
            result["owner"] = owner
        
        return result
        
    except Exception as e:
        return {"success": False, "exists": False, "error": str(e)}
